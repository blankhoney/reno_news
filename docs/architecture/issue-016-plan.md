...
1. [ ] Add DB integration tests for title/source/summary matches, board filtering, hidden filtering, blocked filtering, disabled-source filtering, and no-result behavior.
2. [ ] Implement reader search repository method over reader-safe fields.
3. [ ] Add API tests for `GET /reader/search`, board filter, missing query, and empty query.
4. [ ] Implement the Fastify route with full JSON Schema query validation.
5. [ ] Add web API client tests for search query URL construction.
6. [ ] Add `/search` reader page and a search form entrypoint from reader surfaces.
7. [ ] Update README, Reader API docs, master plan, and log.
