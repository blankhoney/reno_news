...
- [x] Add DB integration tests for digest item visibility, board filtering, bounded limit, hidden raw-entry exclusion, blocked item exclusion, disabled-source exclusion, and empty-result behavior.
- [x] Implement reader digest repository method over reader-safe fields.
- [x] Add API tests for `GET /reader/digest`.
- [x] Implement `GET /reader/digest` with Fastify v5 full JSON Schema query validation.
- [x] Add web API client tests for digest loading.
- [x] Add `/digest` reader page and navigation entrypoint.
- [x] Update README, Reader API docs, master plan, and log.
...
Implemented:
- `ReaderRepository.listReaderDigestItems` over reader-safe item card fields.
- `GET /reader/digest` with optional board and bounded limit validation.
- Web API helper, `/digest` page, and reader navigation entrypoints.
- README, Reader API docs, Issue 019 plan, implementation log, master plan, and goal plan updates.

Validated:
- DB integration tests cover digest visibility, board filtering, bounded limit, hidden raw entries, blocked items, disabled-source items, and empty board results.
- API tests cover digest item loading, optional board and limit, and invalid limit.
- Web tests cover digest URL construction and response handling.
- Full repo install, lint, tests, and build pass.
- Worker discovery tests and worker lock consistency pass.
- Compose service status, direct health checks, and Caddy-proxied health checks on port `8080` pass.
- Current-code smoke on API `3101` and web `3100` confirms `/reader/digest?board=ai&limit=3` and `/digest?board=ai` surface the seeded AI digest item.
- Local smoke servers were stopped, ports `3100` and `3101` are clear, and `apps/web/next-env.d.ts` has no diff after the final web build.
...
- [x] Add DB integration tests proving Digest ordering lowers otherwise comparable items with higher Quality Feedback Penalty.
- [x] Cover penalty clamping and all current Feedback Types.
- [x] Cover that hidden raw entries, blocked items, and disabled-source items remain excluded by existing visibility filters.
- [x] Implement the computed penalty inside `ReaderRepository.listReaderDigestItems`.
- [x] Keep `GET /reader/digest` response shape unchanged.
- [x] Update README, Reader API docs, master plan, goal plan, and implementation log.
...
Implemented:
- `ReaderRepository.listReaderDigestItems` now computes a bounded Quality Feedback Penalty from existing `reader_feedback` rows.
- Digest preview ordering lowers otherwise comparable items with quality, correction, duplicate, broken-link, or rights-concern feedback.
- `GET /reader/digest` keeps the same response shape; the penalty is not exposed to readers.
- README, Reader API docs, Issue 020 plan, implementation log, master plan, and goal plan updates.

Validated:
- DB integration test first failed before implementation because the penalized item still ranked ahead of a clean item.
- DB integration tests now cover all current Feedback Types, penalty clamping, ordering impact, and existing hidden/blocked/disabled-source filtering.
- API tests confirm `GET /reader/digest` keeps its response shape.
- Full repo install, lint, tests, and build pass.
- Worker discovery tests and worker lock consistency pass.
- Compose service status, direct health checks, and Caddy-proxied health checks pass.
- Current-code smoke on API `3101` and web `3100` confirms the clean digest item ranks and renders before the penalized item.
- Issue 020 smoke rows were deleted, ports `3100` and `3101` are clear, and `apps/web/next-env.d.ts` has no diff after the final web build.
...
- [x] Add script-contract tests for backup and restore drill command boundaries.
- [x] Add local backup script or package script for timestamped custom-format dumps.
- [x] Add local restore-drill script or package script for disposable database restore verification.
- [x] Add `docs/ops/backup-restore.md`.
- [x] Update README, master plan, goal plan, and implementation log.
- [x] Run a local backup/restore drill and clean up the disposable restore target and dump file.
...
Implemented:
- Root scripts `db:backup:local` and `db:restore:drill`.
- `scripts/db-backup.sh` for timestamped PostgreSQL custom-format Backup Snapshots.
- `scripts/db-restore-drill.sh` for disposable database restore verification with `pg_restore --exit-on-error`.
- `docs/ops/backup-restore.md` runbook.
- README, Issue 022 plan, implementation log, master plan, and goal plan updates.

Validated:
- Script-contract tests verify `pg_dump -Fc`, `pg_restore --exit-on-error`, non-interactive Compose execution, ignored `backups/`, and disposable restore safeguards.
- `pnpm db:backup:local` created `backups/reno_news-20260519T224137Z.dump`.
- `pnpm db:restore:drill backups/reno_news-20260519T224137Z.dump` restored into disposable database `reno_news_restore_drill`, verified expected MVP tables, and dropped the disposable database on exit.
- The temporary dump file was deleted and `reno_news_restore_drill` no longer exists.

Known limitations:
- This is a local manual logical dump and restore drill only. It intentionally does not add production backup scheduling, remote object storage, WAL archiving, point-in-time recovery, monitoring integrations, alerting, production credentials, release workflow, auth/RBAC, Admin identity, audit logs, semantic/vector search, external search services, digest delivery, persisted digest tables, editorial workflow, browser automation, or non-RSS adapters.

#### Issue 023: Add local release health audit foundation
...
Let an operator run a local Release Health Audit before treating the current build as releasable, without adding a deployment workflow, remote monitoring integration, alerting, or production automation.

Required Tasks

- [x] Research Docker Compose service status/health output and GitHub Actions permission guidance relevant to release audit boundaries.
...
- [x] Add script-contract tests for Disk Usage Guard command boundaries and forbidden destructive operations.
- [x] Add Compose logging limits for all services.
- [x] Add a root package script for a local read-only disk usage check.
- [x] Implement a local disk usage check script that reports Docker disk usage and local backup artifact size.
- [x] Add `docs/ops/disk-usage.md` with prerequisites, expected evidence, failure handling, manual cleanup references, and limitations.
- [x] Update README, master plan, goal plan, and implementation log.
- [x] Run the disk usage check locally and verify Compose config still renders.
...
Implemented:
- Compose `json-file` logging limits for web, API, worker, scheduler, PostgreSQL, Redis, and Caddy.
- Root script `disk:check:local`.
- `scripts/disk-usage-check.mjs` for read-only Docker disk usage and local backup artifact inspection.
- `docs/ops/disk-usage.md` runbook.
- README, Issue 024 plan, implementation log, master plan, and goal plan updates.

Validated:
- Script-contract tests verify logging limits, read-only disk check behavior, runbook sections, and forbidden destructive operations.
- `pnpm disk:check:local` completed successfully and ended with `Disk Usage Guard OK`.
- `docker compose -f infra/compose/compose.yml config` rendered successfully with logging limits.
...
|---|---|---|---|
| V2.1 | Identity, RBAC, and Audit | Completed | API-owned sessions, reader/admin roles, route guards, and audit events are covered by tests. |
...
| V2.5 | Account-Backed Personal State and Digest Editions | Completed | saved/read-later/read_status are account-backed, local state can migrate, and digest editions are persisted and replayable. |
| V2.6 | Structured Source Expansion and Similarity Signals | In Progress | GitHub and arXiv adapters enter the existing pipeline safely; GDELT and RSSHub stay constrained; PostgreSQL remains the search source of truth. |
